---
title: "Advanced Inference: 2 - Cross-Validation"
output:
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
crossVal <- function(dataset, k=1) {
  ...
  for(fold in 1:K) {   
    # Select the row 
    val_idx <- fold
    V <- dataset[c(val_idx),]          # Construct the validation set
    DminusV <- dataset[-c(val_idx),]   # Construct the training set
    ...
    # Compute the CV-SSE:
    m1_cvsse <- sum((m1$residuals)**2)
    m2_cvsse <- sum((m2$residuals)**2)
...
crossVal(demographics, k=1)
```

```{r mc1}
question("Based on the CV-SSE, which model do you think performs better on the demographics data?",
...
```{r ex1, exercise=TRUE}
crossValIce <- function(dataset, k) {
...
crossValIce <- function(dataset, k) {
  ...
  # Matrix of results
  results <- matrix(nrow=K, ncol=3)
  colnames(results) <- c("fold", "m1-CV-SSE", "m2-CV-SSE")
  ...
  for(fold in 1:K) {   
    ...
    # Fit model 1 without interactions
    m1 <- lm(data=dataset, cons ~ temp + price + income)
    
    # Fit model 2 with interactions
    m2 <- lm(data=dataset, cons ~ temp + price + income + price:income)
