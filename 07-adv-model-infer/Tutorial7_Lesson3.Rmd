...
In this tutorial, you'll learn more about inference and prediction for regression models. Our main goal is to learn how to quantify uncertainty not just in our model parameters, but also in the inferences and predictions that we make with them. To start, let's review the workflow for estimating parameters. 


### Review: estimate parameters and quantify uncertainty
...
So, our flight from NYC to BOS may take *about* 42.39 minutes, plus or minus some error term. We'll need to adapt our tools to handle the uncertainty we have about this error term - more on that below.
...
Recall from Tutorial 5 the lesson on bootstrapping: from our data sample, we generated more data by sampling from the original dataset with replacement. If we repeat that many times for this example, we can show how the models change with each new sample set.


```{r}
boots <- function(data, n=15000) {
  ...
  binter <- bmodel$coefficients[["(Intercept)"]]
  bslope <- bmodel$coefficients[["distance"]]
  
  # TODO: how to plot diff models?
  # TODO: need SEs as well
  # TODO: error distribution = epsilon dist, Gaussian w sigma^2 hat
}
```
