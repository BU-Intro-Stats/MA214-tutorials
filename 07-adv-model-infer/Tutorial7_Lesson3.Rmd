...
nycflights$air_time <- nycflights$air_time + rnorm(n=nrow(nycflights))
head(nycflights)
```
...
As we would expect, there is a strong correlation ($p=2.0\times10^{-16}$) between flight distance and air time. The average increase of $0.1269$ minutes is a point estimate based on a sample of $n=40$ flights. Given the model output above and any relevant R functions, construct a $95\%$ confidence interval to give an interval estimate of the true $\beta_1$:
...
For example, there are three rows in the dataset corresponding to a flight of distance 200 miles:

```{r}
subset(nycflights, distance == 200)
```

Let's plot these with our line:

```{r}
subset <- subset(nycflights, distance == 200)
subset$yhat <- 18 + 0.12*(subset$distance)  # FIXME: these should be different values

ggplot(data=subset, aes(x=distance)) +
  geom_point(aes(y=air_time)) +
...
# Generate fitted models from 1000 bootstrapped estimates:
boots <- nycflights |>
...
# TODO: calculate SE for each param and overall SE
#summarize(boots, se=sd(stat))  # compute the SE
```
