...
Now let's look at the boostrap models' residual SE, and the SE of each bootstrapped parameter:
...
Now load the data, fit a linear model, and output the model summary. Note the parameter and residual SE here - we will return to these values later. 
...
```{r mc5, echo=FALSE}
question("How do you compute the SE for each parameter?",
...
And now we compare the SE for our bootstrapped models vs our baseline model:
...
Recall our formula for the SE of a prediction $\hat{y}^* = \beta_0 + \beta_1 x^*$ on a new value $x^*$:
...
Now we'll walk through computing the SE for each of your 3 predictions above, step by step. To start, let's compute the first term, $s^2_e$
