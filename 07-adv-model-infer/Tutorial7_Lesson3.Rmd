...
As we would expect, there is a strong correlation ($p=2.0\times10^{-16}$) between flight distance and air time. The average increase of $0.126$ minutes is a point estimate based on a sample of $n=40$ flights. Given the model output above and any relevant R functions, construct a $95\%$ confidence interval to give an interval estimate of the true $\beta_1$:
...
For example, there are two rows in the dataset corresponding to a flight of distance 200 miles:

```{r}
subset(nycflights, distance == 200)
```
Note how different the response values $y$ are! To visualize this, let's plot our regression line and the two `distance=200` points below: 
...
subset <- subset(nycflights, distance == 200)  
...
So, we'll need to adapt our tools to quantify the uncertainty we have about this error term - more on that below.
...
The uncertainty here, shown by the marked difference between our predicted values (blue line) and the observed values (points), is determined by the `residuals` term in our model. In the next section, we'll use different approaches to analyze this uncertainty.


## Bootstrapping regression estimates

***TODO: insert introduction/writeup for this section***
...
bPredictions <- matrix(nrow=nrow(bIntercepts)*nrow(nycflights), ncol=3)
colnames(bPredictions) <- c("distance", "yhat", "bootstrapLine")
...
# Compute the residuals of the models
residuals <- matrix(nrow=nrow(nycflights),ncol=1)
colnames(residuals) <- c("residuals")
residuals <- data.frame(residuals)

# FIXME: indexing of model params is incorrect - not the same as indexing data
for (model in 1:nrow(bIntercepts)) { # 1:nrow(nycflights)
  yhat <- bIntercepts$estimate + (bPredictions$distance * bSlopes$estimate)
  #yhat <- bIntercepts[i,"estimate"] + bSlopes[i,"estimate"]*nycflights[i,"distance"]
  #e <- nycflights[i,"air_time"] - yhat
  #residuals[i, "residuals"] <- e
}

# Compute the residual SE
se_resid <- sd(residuals$residuals)
```
