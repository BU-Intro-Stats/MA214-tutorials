...
$\hat{y_i} = \beta_0 + \beta_{\rm temp}x_{i,\rm temp} + \beta_{i,\rm price}x_{i,\rm price} + \beta_{i,\rm income}x_{\rm income} + \epsilon_i$

with residuals $\epsilon_i$. Now, fit this model below:
...
***FIXME: below hints don't render with comments, but without comments they throw a parsing error***

```{r ex2-hint1}
# Use the lm() command with the data you loaded:
m1 <- lm(data=icecream, ____)
```

```{r ex2-hint2}
# `cons` is the dependent (response) variable, denoted by Y ~
m1 <- lm(data=icecream, cons ~ ____)
```
...
$\hat{y_i} = \beta_0 + \beta_{\rm temp}x_{i,\rm temp} + \beta_{\rm price}x_{i,\rm price} + \beta_{\rm income}x_{i,\rm income} + \beta_{\rm price:income}x_{i,\rm price}x_{i,\rm income} + \epsilon_i$
