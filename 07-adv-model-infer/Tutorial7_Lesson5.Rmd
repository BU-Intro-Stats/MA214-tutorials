...
While odds are a useful concept, they can be hard to work with in modeling contexts. Consider: odds are bounded in the interval $[0,1]$. What if we take the $\rm log$ of it? This function is called (predictably) the log-odds:

$$ {\rm log} \biggl( \frac{p_i}{1-p_i} \biggr) = \beta_0 + \beta_1 X_{i,1} + \beta_2 X_{i,2}$$
and the log-odds function is not bounded: it maps a probability $p\in[0,1]$ to the entire real line: $(-\infty, +\infty)$. Now a revised table interpreting both odds and log-odds values looks like this:
...
***TODO: (concept check MC) ask what is the probability of a low birthweight if mom's weight did not change but she is a smoker? and, same setup but non-smoker?***
