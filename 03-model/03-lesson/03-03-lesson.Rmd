...
1. $Y$ is changed to $\widehat{y}$, as we are now estimating the mean response rather than an individual response. 
2. The error terms ($\epsilon$) are removed, as we are no longer modeling individual responses. 
3. The $\beta_0$ is replaced with $\widehat{\beta}_0$, notating that this is an estimate of the true intercept. 
4. The $\beta_1$ is replaced with $\widehat{\beta}_1$, notating that this is an estimate of the true slope.
...
- $\widehat{Y}$ is the mean value of the response, for a given value of $X$.
  * $\widehat{Y}$ is our best guess for the true value of $Y$ given what we know about $X$.
- The $\widehat{\beta}$s are estimates of true, unknown $\beta$s.
...
Which of the following corresponds to the slope?
...
",
  answer("exactly 11.7%", message = "Hmmm...do we know the mean poverty rate exactly?"),
  answer("exactly 10.0%", message = "Hmmm...do we know the mean poverty rate exactly?"),
...
Two facts enable you to compute the estimated slope $\widehat{\beta}_1$ and intercept $\widehat{\beta}_0$ of a simple linear regression model from some basic summary statistics.

First, the slope satisfies the relationship

$$
    \beta_1 = r_{x,y} \cdot \frac{s_y}{s_x}
$$
...
The estimated slope and intercept must satisfy the same two conditions, with all quantities ($s_x$, $s_y$, $r_{x,y}$, $\bar x$, $\bar y$) computed from the data. 
In this exercise, you will find the slope and intercept of the least squares regression line for body weight ($y$) as a function of height ($x$). But, you might need to do some algebra to solve for $\widehat{\beta}_0$!
...
```{r mc3}
question("What are the slope and intercept estimates?",
  answer("$\\widehat{\\beta}_1 = 0.5, \\widehat{\\beta}_0 = 136$", message = "Double check your slope calculation"),
  answer("$\\widehat{\\beta}_1 = 0.5, \\widehat{\\beta}_0 = -17$", message = "Double check your calculations"),
  answer("$\\widehat{\\beta}_1 = 1.0, \\widehat{\\beta}_0 = -105$", correct = TRUE, message = "Nice work"),
  answer("$\\widehat{\\beta}_1 = 1.0, \\widehat{\\beta}_0 = 101$", message = "Double check your intercept calculation"),
  allow_retry = TRUE
)
```
