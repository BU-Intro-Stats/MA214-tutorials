---
title: "Regression modeling: 5 - Assessing model fit"
output:
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
A sensible choice would be the average length of all possum. And in fact, if you have to make the same prediction for every possum, the average is the *best* number you can pick. We can think about this as a model where $\widehat{y}$ (the predicted value of $y$) is equal to $\bar{y}$ (the average value of $y$).

### Visualization of base model

This model is often called the **base model** (or **null model**, but we prefer to avoid the hypothesis testing baggage of the "null" terminology).
This model corresponds to a regression model without any predictors, so it makes sense to use as a benchmark: it doesn't require any insight to make and is a direct simplification of the model of interest (which *does* have the predictor `total_l`). The following code fits the model without any predictors and plots the residuals:

```{r possum-base}
base_mod <- lm(total_l ~ 1, data = possum)
...
### SSE of the base model

We can fit the base model in R using `lm()`, but including only the constant 1 as our explanatory variable. This results in an SSE value of 1913.826.

```{r possum-base-summary}
possum_base <- lm(total_l ~ 1, data = possum)
```
...
The ratio of the SSE for our model to the SSE for the base model is a quantification of the variability explained by our model. More specifically, the SSE for the base model is often called SST, for the *total* sum of the squares. This is a measure of the variability in the response variable.
...
The $R^2$ gives us a numerical measurement of the strength of fit of our linear regression relative to a base model based on the average of the response variable ($\widehat{y}_{base} = \bar{y}$).

The base model has an $R^2$ of zero because $SSE = SST$. That is, since the fitted values ($\widehat{y}_{base}$) are all equal to the average ($\bar{y}$), the residual for each observation is the distance between that observation and the mean of the response. Since we can always fit the base model, it serves as a baseline against which all other models will be compared.


### Use $R^2$, not correlation

As a rule, correlation gives too optimistic picture of the strength of the association:

```{r correlation-r-squared-plot, echo=FALSE}
df <- data.frame(
...
ggplot(df, aes(x = y, y = x)) +
  geom_line() +
  labs(x = "r", y = expression(R^2))
```

For example, a correlation of -0.5 corresponds to $R^2 = 0.25$. Even a seemingly strong correlation of $0.7$ corresponds to an $R^2$ value of 
less than 0.5 ($R^2 = 0.49$). 
So, in summary $R^2$ has the following benefits compared to $r$:

1. $R^2$ has a clear statistical interpretation in terms of proportion of explained variance whereas $r$ does not.
2. $R^2$ is applicable to scenarios multiple predictors or even when using more complex models like neural networks. 
3. $R^2$ gives a more accurate numerical summary of the strength of the association, whereas $r$ overstaets the association.

Therefore, **you should always use $R^2$ rather than $r$**. 
...
While $R^2$ is certainly a useful and ubiquitous measure of model fit, it is not the be-all-and-end-all of statistical modeling. A high $R^2$ alone doesn't mean that you have a "good" model, and low $R^2$ doesn't mean that you have a lousy model. A model with a high $R^2$ may be overfit, or it may violate the conditions for inference that we will discuss later lesson. A model with a low $R^2$ can still provide substantial insight into a complex problem.
...
As noted previously, observations of high leverage may or may not be *influential*. The influence of an observation depends not only on its leverage, but also on the magnitude of its residual. Recall that while leverage only takes into account the explanatory variable ($x$), the residual depends on the response variable ($y$) and the fitted value ($\widehat{y}$).
