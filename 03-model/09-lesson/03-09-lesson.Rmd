...
**Exercise:** The medical school acceptance data is loaded as `MedGPA`.
...
In the previous exercise, we identified a major limitation to fitting a linear regression model when we have a binary response variable. However, it is not *always* inappropriate to do so. Note that our regression line only makes illogical predictions (i.e. $\widehat{y} \lt 0$ or $\widehat{y} \gt 1$) for students with very high or very low GPAs. For GPAs closer to average, the predictions seem fine. 

Moreover, the alternative logistic regression model  -  which we will fit next  -  is very similar to the linear regression model for observations near the average of the explanatory variable. It just so happens that the logistic curve is very straight near its middle. Nevertheless, you should never use a linear regression model to fit data with binary responses.
...
$$
y \sim \mathrm{Bernoulli}(p), \quad\quad \text{where}\quad    \log{ \left( \frac{p}{1-p} \right) } = \beta_0 + \beta_1 \cdot x.
$$


**Exercise:** Use `glm()` to fit a logistic regression model for `Acceptance` as a function of `GPA`.
...
- Use `geom_smooth()` to add the logistic regression line by specifying the `method` and `method.args` arguments to fit a logistic `glm`.
...
One of the difficulties in working with a binary response variable is understanding how it "changes." The response itself ($y$) is *either* 0 or 1, while the fitted values ($\widehat{y}$) - which are interpreted as probabilities - are *between* 0 and 1. But if every medical school applicant is either admitted or not, what does it mean to talk about the *probability* of being accepted? 
...
Here we are plotting $\widehat{p}$ as a function of $x$:

$$
    \widehat{p} = \frac{\exp{( \widehat{\beta}_0 + \widehat{\beta}_1 \cdot x )}}{1 + \exp( \widehat{\beta}_0 + \widehat{\beta}_1 \cdot x ) } \,.
$$

Note that the left hand side is the estimated probability $\widehat{p}$ of being accepted to medical school.

**Exercise:**

- Create a scatterplot for `acceptance_rate` as a function of `mean_GPA` using the binned data in `MedGPA_binned`. Use `geom_line()` to connect the points.
...
$$ 
odds(\widehat{p}) = \frac{\widehat{p}}{1-\widehat{p}} = \exp{( \widehat{\beta}_0 + \widehat{\beta}_1 \cdot x ) }
$$
...
$$ 
logit(\widehat{p}) = \log{ \left[ \frac{\widehat{p}}{1-\widehat{p}} \right] } = \widehat{\beta}_0 + \widehat{\beta}_1 \cdot x
$$
...
$$ 
OR = \frac{ odds(\widehat{y} | x + 1 )}{odds(\widehat{y} | x )} 
  = \frac{ \exp{( \widehat{\beta}_0 + \widehat{\beta}_1 \cdot (x + 1) ) }}{\exp{( \widehat{\beta}_0 + \widehat{\beta}_1 \cdot x ) }} 
  = \exp{\beta_1} 
$$
...
If the probability of getting accepted is $p$, then the odds are $p / (1-p)$. Expressions of probabilities in terms of odds are common in many situations, perhaps most notably gambling. 

Here we are plotting $p/(1-p)$ as a function of $x$, where that function is
...
For the odds, first try thinking of how the acceptance rate and odds are related. Then, use `mutate()` to do the computation.
</details>

<details>
<summary>**Hint 2**</summary>
For computing the observed odds, what output from the model do you think you could use?
</details>
...
1. on the probability scale, the units are easy to interpret, but the function is non-linear, which makes it hard to understand
2. on the odds scale, the units are harder (but not impossible) to interpret, and the function is exponential, which makes it harder (but not impossible) to interpret

We'll now add a third formulation:

3. on the log-odds scale, the units are nearly impossible to interpret, but the function is linear, which makes it easy to understand
...
$$
    OR = \frac{odds(\widehat{p} | x + 1 )}{ odds(\widehat{p} | x )} = \exp{\beta_1}
$$
...
Our fitted medical school logistic regression model is given by the following:

$$
odds(\widehat{p}) = e^{\widehat{\beta_0} + \widehat{\beta_1}x} = e^{-19.21 + 5.45x}
$$
Say that Donald, a student applying to medical schools, has a GPA of $2.9$. Use the fitted model above to answer the following questions.

```{r mc1}
question("What are Donald's odds of being accepted into medical school, according to our model?",
  answer("e^{15.81}"),
  answer("e^{-3.41}", correct = TRUE),
  answer("e^{-19.21}"),
  answer("e^{5.45}"),
  allow_retry=TRUE
)
...
<details>
<summary>**Hint**</summary>
Use the odds equation: $odds(\widehat{p})=\frac{\widehat{p}}{1-\widehat{p}}$ 
</details>

Say that Donald (allegedly) hacks into his school's registrar and changes his GPA to 3.9. 

```{r mc2}
question("Now what is the probability that Donald is accepted into medical school, according to our model?",
  answer("3.2%"),
  answer("< 0.1"),
  answer("99%"),
...
<!-- The fitted coefficient $\widehat{\beta}_1$ from the medical school logistic regression model is 5.45. The exponential of this is 233.73.  -->
...
So how well did our model perform? One common way of assessing performance of models for a categorical response is via a confusion matrix. This simply cross-tabulates the reality with what our model predicted. In this case, our model predicted that 91 patients would die, and only 12 would live. Of those 91, 71 actually did die, while of the 12, 8 actually lived. 

<!-- Thus, our overall accuracy was 79 out of 103, or about 77%.  -->

```{r mc2}
question("What is the overall accuracy of the model?",
  answer("71 out of 91, or about 78%."),
  answer("8 out of 12, or about 67%."),
  answer("79 out of 103, or about 77%.", correct = TRUE),
  allow_retry=TRUE
)
```
...
Naturally, we want to know how well our model works. Did it predict acceptance for the students who were actually accepted to medical school? Did it predict rejections for the student who were not admitted? These types of predictions are called *in-sample*. As before, we can evaluate the binary response model with a confusion matrix.
...
**Exercise:** The model object `mod` is already loaded for you. 
...
<details>
<summary>**Hint 1 **</summary>
Try using `augment()` and `mutate()` to create `tidy_mod`.
</details>

<details>
<summary>**Hint 2**</summary>
For the confusion matrix, you'll want the output to be in a `table()`, which you can use the last line to specify.
</details>
