...
The output from `lm()` reminds us how we called it, and then gives us the fitted coefficients. The value of the intercept coefficient $\widehat{beta}_0$ is \$0.93, while the value of the slope coefficient $\widehat{\beta}_1$ is \$1.20. 

### Regression equation

$$
  \widehat{\mathrm{ucla_{new}}} = 0.929 + 1.199 \cdot \mathrm{amazon_{new}}
$$
...
$$
    \widehat{\mathrm{poverty}} = 64.594 - 0.591 \cdot \mathrm{hs_{grad}}
$$
...
$$
    \widehat{\mathrm{wgt}} = -105.011 + 1.018 \cdot \mathrm{hgt}
$$
and

$$
    \widehat{\mathrm{slg}} = 0.009 + 1.110 \cdot \mathrm{obp}
$$

```{r mc3}
question("Which of the following statements is **incorrect**?",
  answer("Because the slope coefficient for `obp` is larger (1.110) than the slope coefficient for `hgt` (1.018), we can conclude that the association between `obp` and `slg` is stronger than the association between height and weight. ", correct = TRUE, message = "We cannot conclude anything about the strength of the association by examining the slope coefficients."),
  answer("A person who is 170 cm tall is expected to weigh about 68 kg.", 
         message = "This answer is correct! If you plug in the values, a person who is 170 cm tall *is* expected to weigh about 68 kg: -105.011+ (1.018 * 170) = 68.049."),
  answer("A player with an on base percentage of 0.350 is expected to have a slugging percentage of 0.509.", 
         message = "This answer is correct! If you plug in the values, a player who has a 0.350 OBP *is* expected to have a slugging percentage of 0.509: 0.009 + 1.110 * 0.350 = 0.509."),
  answer("None of the above.", message = "Try again. One of the statements is incorrect."),
  allow_retry = TRUE
)
```
...
Since the object `books_mod` contains everything R knows about our model, we can ask R for the fitted values, using the `fitted.values()` function. This returns a vector containing the $\widehat{y}$ values for each observation in our dataset.

In general, the length of the vector of fitted values is the same as the number of rows in the original data frame, since each observation corresponds to exactly one value of $\widehat{y}$. However, if there were any observations with missing data, those will be automatically discarded by R when the model is fit, and thus, the length of the vector of fitted values may not be as large as the number of rows in the original data frame. 
...
Once you have fit a regression model, you are often interested in the fitted values ($\widehat{y}_i$) and the residuals ($e_i$), where $i$ indexes the observations. Recall that:
...
In this exercise, we will confirm these two mathematical facts by accessing the fitted values and residuals with the `fitted.values()` and `residuals()` functions, respectively, for the `hgt_wgt_mod` model.
We will use the `all.equal()` function to confirm that the mean of the body weights equals the mean of the fitted values of `hgt_wgt_mod`. This function takes two inputs (separated by a comma) that you would like to check if they are equal (up to a numerical tolerance of $1.5 \times 10^{-8}$). 
...
As you fit a regression model, there are some quantities (e.g. $R^2$) that apply to the model as a whole, while others apply to each observation (e.g. $\widehat{y}_i$). If there are several of these per-observation quantities, it is sometimes convenient to attach them to the original data as new variables.
