...
# source: https://stats.libretexts.org/Bookshelves/Applied_Statistics/Biostatistics_-_Open_Learning_Textbook/Unit_1:_Exploratory_Data_Analysis/Case_Q-Q
...
```{r ex5-hint-1}
ggplot(data = bdims, aes(x = ___, y = ___, color = as.factor(sex)) +
  geom_point()
```
...
### Exploring different scalings

Now, using the `mlbbat10` dataset, explore whether using the `scale_x_log10()` and `scale_y_log10()` functions (or just one of them),
results in the scatterplot of on-base percentage (`obp`) versus slugging percentage (`slg`) is most linear. 

```{r ex8b, exercise = TRUE}
ggplot(data = mlbbat10, aes(x = ___, y = ___)) +
  geom_point() +
  ___ 
```
...
### 
```{r mc-ex8b}
question("Which rescaling gives the most linear relationship?",
  answer("log scale for `obp` only", message = "Hmm...did you try all the transformations?"),
  answer("log scale for `slg` only", message = "this is very nonlinear!"),
  answer("log scale for `obp` and `slg`", correct = TRUE, message = "The scatterplot shows the strongest linear relationship!"),
  answer("no rescaling", message = "It's pretty good. But is it the best?"),
  allow_retry = TRUE
)
```
...
```{r mc-ex10}
question("Who is the outlying player?",
  answer("P Feliz", message = "Nope"),
  answer("B Wood", correct = TRUE, message = "You got it!"),
  answer("J Mathis", message =  "Nope"),
  answer("A Moore", message =  "Nope"),
  allow_retry = TRUE
)
```
