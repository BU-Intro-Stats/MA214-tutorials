...
In this tutorial, we will learn how to extend simple linear regression to an arbitrary number of explanatory variables, which can be a combination of numeric and categorical.
...
$$
\mathrm{hwy} = \beta_0 + \beta_1 \cdot \mathrm{displ} + \beta_2 \cdot \mathrm{year} + \epsilon
$$
...
$$
\mathrm{newer} = \begin{cases} 1 & \text{if } \mathrm{year} = 2008 , \\\\  0 & \text{if } \mathrm{year} = 1999 \end{cases}
$$ 
Using `displ` and `newer` as the two explanatory variables, the model is 
$$
\widehat{\mathrm{hwy}} = \widehat{\beta}_0 + \widehat{\beta}_1 \cdot \mathrm{displ} + \widehat{\beta}_2 \cdot \mathrm{newer}.
$$ 
Now we compute the fitted coefficients for this model using `lm()`. 
...
With the model parameters in hand, we're going to make use of a little high school algebra to inform our understanding of the geometry of our model. 
For `year = 2008`, the model is 
$$\begin{align}
\widehat{\mathrm{hwy}} 
&= 35.276 - 3.611 \cdot \mathrm{displ} + 1.402 \cdot (1)\\
& = \left( 35.276 + 1.402 \right) - 3.611 \cdot \mathrm{displ} \\
&= 36.678 -  3.611 \cdot \mathrm{displ}.
\end{align}$$ 
For `year = 1999`, the model is 
$$\begin{align}
\widehat{\mathrm{hwy}}
&= 35.276 - 3.611 \cdot \mathrm{displ} + 1.402 \cdot (0) \\
&= 35.276 - 3.611 \cdot \mathrm{displ}.
\end{align}$$ 


So what happens when the cars are newer? We get an equatioun for a line. Note that since `displ`acement is our numeric explanatory variable, the slope of the line is -3.611 mpg per liter, and the intercept is 36.678---the sum of the other two coefficients.

What about the older cars from 1999? In that case, the value of `newer` is 0, and plugging that in to our equation also results in the equation for a line. This line also has a slope of -3.611 mpg per liter, but now the intercept is just 35.276 mpg.
...
So, to summarizze, we can write the linear equations for the new cars as 
$$
\begin{aligned}
    \widehat{\mathrm{hwy}} &= ( \widehat{\beta}_0 + \widehat{\beta}_2 ) + \widehat{\beta}_1 \cdot \mathrm{displ} \\\\
    &= \, \stackrel{\mathrm{intercept}}{36.678} - \stackrel{\mathrm{slope}}{3.611} \cdot ~ \mathrm{displ}
\end{aligned}
$$
and for the old cars as
$$
\begin{aligned}
    \widehat{\mathrm{hwy}} &= \widehat{\beta}_0 + \widehat{\beta}_1 \cdot \mathrm{displ} \\\\
    &= \, \stackrel{\mathrm{intercept}}{35.276} - \stackrel{\mathrm{slope}}{3.611} \cdot \mathrm{displ}
\end{aligned}
$$
...
Applying `augment()` to our model will return a data frame with the fitted values attached, like the one you see here:
...
```
Note that the name for the variable that contains the fitted values is `.fitted`, and the name of the categorical variable is ``factor(year)``.
...
Note that this approach has the benefit of automatically coloring the lines appropriately to match the data.  

### Your turn

<!-- You already know how to use `ggplot()` and `geom_point()` to make the scatterplot. The only twist is that now you'll pass your `augment()`-ed model as the `data` argument in your `ggplot()` call. When you add your `geom_line()`, instead of letting the `y` aesthetic inherit its values from the `ggplot()` call, you can set it to the `.fitted` column of the `augment()`-ed model. This will automatically color the lines for you. -->

Coming back to the `mariokart` data, try to plot the parallel slopes model.
The fitted model `mod` relating total price (`total_pr`) to the number of `wheels` and `cond`ition is already in your workspace:

- Call the `augment()` function on the model object `mod`, then use `glimpse(augmented_mod)` to explore the resulting data frame.
- To create `data_space` first map each of the three variables to an aesthetic in your `ggplot()`, then use the `geom_jitter()` function
with `width = 0.2`, `height = 0`, and `alpha = 0.5`. 
...
```{r ex2-hint-2}
data_space <- ggplot(augmented_mod, aes(x = wheels, y = ___, color = ___)) + 
  geom_jitter(width = 0.2, ___)
```
...
```{r mc2}
question("Why use `geom_jitter` rather than `geom_point`?:",
  answer("`geom_jitter` lets us use transparency (`alpha = 0.5`) to improve readability of the plot.", message="While tranparency does improve readability there, `geom_point` also supports transparency."),
  answer("`geom_jitter` uses larger, more easily seen points.", message="Nope! We didn't adjust the point size, which in any case `geom_point` can do too."),
  answer("`geom_jitter` lets us move the points slightly on the x axis so they are all visible; without the jitter, there are many points for each possible value of `wheel`", correct = TRUE,  message="You got it!"),
  allow_retry=TRUE
)
```
...
- What are the units?  
- What does it mean to be "controlling for"?
...
$$
    \mathrm{birthweight} = \beta_0 + \beta_1 \cdot age + \beta_2 \cdot \mathrm{parity} + \epsilon
$$
...
Use `lm()` to build a parallel slopes model implied by the plot, then plot the data and the two lines.
Make sure to still use `factor()` in this case. 

```{r ex4, exercise=TRUE}
# build model
bwt_mod = lm(___ ~ ___ + ___, data = ___)

# scatterplot, with color
ggplot(augment(___), aes(x = ___, y = ___, color = ___)) + 
...
```{r ex4-hint-1}
# build model
bwt_mod = lm(___ ~ ___ + ___, data = babies)
```

```{r ex4-hint-2}
# scatterplot, with color
ggplot(augment(bwt_mod), aes(x = ___, y = ___, color = ___)) + 
  geom_point(alpha = 0.3) +
  geom_line(aes(y = ___))
```
...
<!-- # scatterplot, with color -->
<!-- ggplot(augment(bwt_mod), aes(x = gestation, y = bwt, color = `factor(smoke)`)) +  -->
<!--   geom_point(alpha = 0.3) + -->
<!--   geom_line(aes(y = .fitted)) -->
<!-- ``` -->
