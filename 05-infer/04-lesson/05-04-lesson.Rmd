...
Benford's Law appears when looking at broad collections of numbers, and paying attention to only to the first digit of each number. For example, consider the populations of every country on earth. This demographic data lives inside the `gapminder` package. We can restrict our attention to data from the year 2007 and select the `country` name and `pop`ulation variables. Now, Benford's Law is only concerned with the first digit, so here we see that that the populations of Afghanistan, Albania, and Algeria all start with three, Angola starts with one, and so on. If we do this for all 142 countries in this data set and visualize the distribution in a bar chart, what do you think it would look like? Would it be uniform?
...
Along the way, you'll use a very useful function called `pivot_longer()`. This allows you to reshape your dataframe by taking information stored in variable names and moving them to live in their own columns in a *longer* data frame.
...
- `pivot_longer()` the candidate names into a new column alongside their respective votes. The names of the previous columns go to a new column called `"candidate"` with the `names_to` argument and the values of the cells go to a new column called `"votes"` with the `values_to` argument. This function also expects you to specify the columns you're pivoting (`cols`) and since we're pivoting the entire dataset of `totals` we can set this argument to `everything()`.
...
# Inspect longer totals
totals_longer
```

```{r ex2-hint}
 Set `cols = everything()`, `names_to = "candidate"`, and `values_to = "votes"`.
```
...
- Use `totals_longer` to construct a bar plot of `votes` to visualize the overall election outcome. Since you already have the totals for each candidate, you need to map `candidate` to the `x`, `votes` to the `y`, then add a `geom_col()` layer.
...
```{r ex3-solution}
# Plot total votes for each candidate
ggplot(totals_longer, aes(x = candidate, y = votes)) +
  # Add col layer
  geom_col()
```
...
Construct a bar plot of `first_digit` in `iowa_county`.
...
```{r ex12-hint}
The three arguments required of the `observe()` function are the `response` variable, the `stat`istic of interest, and vector of probabilities, `p`, which are already saved to your workspace as `p_benford.`
```
