...
Vocabulary scores can only be whole numbers - so what should the binwidth be?
</details>
...
Calculate the standard deviations of each social class. To do this: 
...
<details>
<summary>**Hint**</summary>
Use `specify()`, with `wordsum ~ class`. 
</details>
...
<details>
<summary>**Hint**</summary>
In `calculate()`, your stat should be `F`.
</details>
...
```{r quiz_3}
question("", correct = "Bonferroni would be so proud! The correction factor for $k$ classes is $k * (k - 1) / 2$.", allow_retry = TRUE,
