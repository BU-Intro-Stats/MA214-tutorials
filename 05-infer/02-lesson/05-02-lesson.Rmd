...
It captures how much uncertainty you have in your estimate. The more data that your estimate is based upon (i.e., the larger the sample size), the narrower this distribution and the confidence interval become.
...
A hypothesis test on a proportion asks the question: what sort of $\widehat{p}\text{s}$ would you observe if the true parameter p held a particular value?
...
Thus the sampling distribution, now called the null distribution, captures the variability in $\widehat{p}$ that you might observe in a world where $H_0$ is true.
...
Let’s use a hypothesis test to study another question on the gss. Respondents were asked if they favor or oppose capital punishment, also called the death penalty, for the crime of murder. You can look at the distribution of answers by forming a bar chart. You see that of the 150 respondents, more than half answered in favor.

You can calculate that proportion, $\widehat{p}$, more precisely using the `summarize` command, and you learn that the proportion is about 0.57.
...
If you print out the top of the null data frame, you’ll see in the `stat` column you have 500 examples of the type of $\widehat{p}\text{s}$ that might occur if in fact the true parameter is 0.5. Recall that in the confidence interval setting, these are clustered around the $\widehat{p}$ that was found in the original `gss2016` data set, whereas here they are clustered around the null value 0.5.
...
The distribution of those statistics can be visualized using a density plot. We’ll add in `geom_vline` to overlay a vertical red line at the location of our observed $\widehat{p}$, 0.57. The curve represents the sort of $\widehat{p}\text{s}$ you would see if p was 0.50 and the red line is the $\widehat{p}$ that you actually observed. If your $\widehat{p}$ is in the main part of the null distribution, then it is consistent with the null hypothesis. If it’s far into the tails, though, it’s deemed very unlikely if the null hypothesis were true. 
...
This is formalized by computing the p-value, the proportion of generated $\widehat{p}\text{s}$ that were more extreme than the observed $\widehat{p}$. In other words, we want to summarize the column of statistics by finding the proportion that are greater than 0.57. We multiply that number by two to account for the left tail of the null distribution, which also contained $\widehat{p}\text{s}$ that were just as far from the center of the distribution, in the opposite direction.
...
The null distribution provides a sense of the statistics, the $\widehat{p}\text{s}$, that would be calculated if you were to generate many data sets according to the null hypothesis. The p-value tells you where your actual observed $\widehat{p}$ falls into that distribution. 
...
```{r ex3, exercise=TRUE}
# Generate one data set under $H_0$
sim1 <- gss2016 |>
...
In the last exercise, you got a sense of what a single simulated $\widehat{p}$ might be if in fact the true proportion of believers was 0.75. That $\widehat{p}$ was likely different from the $\widehat{p}$ in `gss2016`, but was that a fluke or is there a systematic inconsistency between that claim and the data in the GSS?
...
Extend your code for the last exercise to `calculate()` sample proportions of believers in 500 generated data sets. Save this collection of $\widehat{p}\text{s}$ to `null`.
...
- Compute the two-tailed p-value (where the proportion of null values is equal to or more extreme than the observed `p_hat`) by multiplying the one-tailed p-value by 2.
...
<details>
<summary>**Hint**</summary>
When the p-value is less than alpha, that's strong evidence that the data is inconsistent with the null hypothesis.
</details>
...
- $H_{0} : p_\mathrm{female} - p_\mathrm{male} = 0$ 
- $H_{A} : p_\mathrm{female} - p_\mathrm{male} \ne 0$ 
...
Let $p$ be the true proportion of individuals who believe in life after death. 
...
We could also phrase it as "The variable postlife is independent from the variable sex." Well, if there truly is no association between our two variables, we should be able to generate more data simply by shuffling or permuting the data.
...
In the last exercise you learned that about 53% of women favor the death penalty while about 63% of men do, a difference of about 10 percentage points. That seems like a large difference, but what if it's just due to chance and in fact there is no relationship between sex and support for the death penalty? Find out by testing the null hypothesis that sex and support for the death penalty are independent of one another.
...
Construct a density plot to visualize the `stat`istics stored in the `null` distribution and add a vertical red line to indicate the observed test statistic `d_hat` that you calculated in the last exercise.
...
Based on the p-value you obtained in the previous exercise, what is your conclusion regarding the association between sex and support for the death penalty? Recall that this code calculates the p-value:

```{r mc2, echo=FALSE}
question("",
  answer("The data are consistent with the hypothesis that there is no association.",correct = TRUE, message="Bingo! Despite the difference that we found between men and women in our sample, it's quite possible it's simply an artifact of the process of drawing a random sample."),
...
<details>
<summary>**Hint**</summary>
Recall that the null hypothesis you are testing is that there is no association between `cappun` and `sex`.
</details>
...
The distribution of the differences in $\widehat{p}\text{s}$ that you might observe in a given sample if the null is true.
...
This is formalized by checking to see if the statistic falls within the area of the tails that account for some proportion, alpha, of the total distribution. It's most common to set alpha to 0.05.
...
If you use alpha equal to 0.05, you're resigning yourself to incorrectly rejecting the null hypothesis 5% of the time.
...
There's a second type of error to be aware of when conducting hypothesis tests, one that occurs when in fact $H_0$ is false and some other model we'll call $H_A$ is the true model.
...
One hopes that after looking at your data and computing a p-value you would decide to reject $H_0$.
...
If you fail to reject $H_0$, you've committed what's called a Type II error.
...
To visualize a Type II error, you have to start by realizing that $H_0$ is no longer the model that is generating your data. Now, it's coming from $H_A$.
...
However, you are using the same thresholds, those dotted gray lines, for the reject and fail-to-reject regions, so you will retain $H_0$ whenever you observe a statistic in this region.
