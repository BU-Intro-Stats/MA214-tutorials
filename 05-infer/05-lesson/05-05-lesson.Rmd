...
First, generate 15000 bootstrap distributions of `rent` in the `manhattan` data frame and record the median of each bootstrap distribution.
...
Now plot a histogram of the bootstrap medians.
...
sample statistic $\pm$ $t_{df = n - 1}^*$ $\times$ $SE\_{boot}$
...
- Use the t-distribution to determine the critical value for a 95% confidence interval.
...
<details>
<summary>**Hint**</summary>
`qt()` takes the percentile of the t-distribution you want to find and the degrees of freedom as arguments.
</details>

<details>
<summary>**Hint**</summary>
The degrees of freedom should be n - 1.
</details>
...
```{r bootstrap_interval_5, echo = TRUE}
...
```{r quiz_1}
quiz(question("Which method is more appropriate for calculating the bootstrap confidence interval for the median rent of 1 BR apartments in Manhattan?",
              correct = "Great job! The standard error method is more appropriate here.", allow_retry = TRUE,
    answer("Percentile method.", message = "This method doesn't always give the best results."),
    answer("Standard error method.", correct = TRUE)), caption = "")
```
...
```{r quiz_2}
quiz(question("Which of the following is **false** about the distribution of `visits`?", correct = "Correct! Filtering the dataset for `is.na(visits)` values reveals missing values for several mothers.", allow_retry = TRUE,
...
Suppose now we're interested in the standard deviation of the number of doctor's visits throughout pregnancy, as opposed to the mean.
...
<details>
<summary>**Hint**</summary>
The steps should all be similar to what you did above, except in `calculate()`, set `stat` to `"sd"`.
</details>
...
Well done! You should now be familiar with how to create bootstrap confidence intervals for the mean and standard deviation of a sample.
...
<!--  Call `hypothesize()`, setting `null` to `"point"` and `med` to `2500`. -->
<!-- ``` -->
...
<details>
<summary>**Hint**</summary>
In `filter()`, try using the condition `stat` greater than or equal to the observed median rent, `rent_med_obs`.
</details>

<details>
<summary>**Hint**</summary>
In `summarize()`, try setting `p_val` to the number of rows, `n()`, divided by the total number of replicates, `n_replicates`.
</details>
...
The p-value you calculated in the previous exercise is available as `rent_p_value`. *TODO: check*
...
```{r quiz_3}
quiz(question("What is the conclusion of the hypothesis test at the 5% significance level?", correct = "Correct! The p-value is above the significance level, so we fail to reject the null hypothesis, and conclude that the data do not provide convincing evidence for the alternative hypothesis.", allow_retry = TRUE,
    ...
    answer("We fail to reject $H_0$ since the p-value is above the significance level, and conclude that the data do not provide convincing evidence that median rent of 1 BR apartments in Manhattan is greater than $2500.", correct = TRUE)), caption = "")
```
...
- Filter for rows where the bootstrap mean is greater than or equal to the observed mean.
...
<details>
<summary>**Hint**</summary>
Use `filter()`, using the condition `stat` greater than or equal to the observed mean weight.
</details>
