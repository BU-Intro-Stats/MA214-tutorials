...
<details>
<summary>**Hint**</summary>
Contingency tables show the counts of different combinations of two variables.
</details>
...
<details>
<summary>**Hint**</summary>
Remember that the null hypothesis in both tests is that there's no relationship between the variables.
</details>
...
- Statistics: $\widehat{p}, \widehat{p}\_{1} - \widehat{p}\_{2}$
...
which shows how we formulate the null distribution as the distribution of test statistics for data generated in a world where the null hypothesis is true. 

The procedure is the same when we're testing independence using the chi-squared statistic, it's just the shape of the null distribution that changes. To formulate a confidence interval for the chi-squared, it makes sense to simply remove the step where we propose a null hypothesis and instead generate data simply through bootstrap resampling...
...
Is the chi squared statistic a estimate of a meaningful population parameter like the mean or the proportion or the difference in two proportions? It turns out the answer is "no": the chi squared statistic is only really useful in the context of a hypothesis test. Constructing a confidence interval wouldn't tell you anything interesting. 
...
So, it's important to not unthinkingly move from hypothesis testing to constructing confidence intervals. For a confidence interval, you need to make sure your statistic is estimating a meaningful popultion quantity, which is not the case here. 
