...
And as with most theorems, this one also only applies if certain conditions are mathematically met. However, it would be incredibly difficult to "prove" that these conditions are mathematically "met," so, instead, we will evaluate the degree to which it appears that these mathematical conditions are violated. 
...
We can create the confidence interval using the `t_test()` function -- don't worry about the word "test" for now -- by using the variable we're interested in as the `response`, specifying that we are interested in a confidence interval by setting `conf_int` to `TRUE`, and setting the confidence level as 0.95 (or 95%).
...
And now you know a range of values you would expect for the true commute time for *all* Americans to take on. Furthermore, if you constructed these intervals for multiple different samples, you would expect that, on average, 95% of them would capture this true commute time.
...
> Can reading and writing scores for a given student be assumed to be independent of each other?
> Probably not! 
...
Since the book price data are *paired* (the price of the same book at the two stores are not independent), rather than using individual variables for the prices from each store, you will look at the single variable of the differences in price. The `diff` column can be calculated as the UCLA Bookstore price minus the Amazon.com price for each book.
...
- Construct a 90% confidence interval for the average difference in prices of the same textbook at the UCLA bookstore and on Amazon.
...
First, for the price differences, generate 1000 bootstrap replicates and calculate their medians. Remember, for the test of a single median you only need to `specify()` a `response` variable. 
