...
To remote the outlier, you could try, for example, a filter on `response` being less than `200`.
</details>

<details>
<summary>**Hint 2**</summary>
To create the plot, add `geom_smooth()`, setting `method` to `"lm"`, `se` to `FALSE`, `data` to `hypdata_no_outlier`, and `color` to `"red"`.
</details>
...
<details>
<summary>**Hint 1**</summary>
Call `lm()` for each dataset to get the predictions. What should your `response` and `explanatory` variables be?
</details>

<details>
<summary>**Hint 2**</summary>
Then, call `tidy()` on each model to get the coefficient-level information. You can wrap the two function calls!
</details>
