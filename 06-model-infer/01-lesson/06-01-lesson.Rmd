...
Indeed, when we take repeated samples of size 20 (here we took 50 different samples), every single line is different.  The `rep_sample_n` command lets take many samples of size 20 and ggplot fit the linear model separately for each of those samples.
...
We can characterize the sampling distribution of the slopes by making a density plot  showing the variability associated with the different slopes.  The R code uses the `tidy` function in the **broom** package to pull out the slope coefficient for each of the separate models.  Using ggplot, we can plot the different slope estimates.
...
- The `RailTrail` data is already loaded and contains information about the number of users of a trail in Florence, MA and the weather for each day.
...
If the original research question had been one sided (that is, are protein and carbs positively associated), the p-value should be divided by two to arrive at a one sided p-value of about 0.015. 
...
The last three exercises have demonstrated how the variability in the slope coefficient can change based on changes to the population and the sample.  


```{r option-111, echo=FALSE}
question("Which of the following combinations *increases* the variability in the sampling distribution of the slope coefficient?",
  answer("Bigger sample size, larger variability around the line, increased range of explanatory variable", message="No, larger sample sizes and increased variability in the explanatory variable both lead to less variability of the slope coefficient."),
  answer("Bigger sample size, larger variability around the line, decreased range of explanatory variable.", message="No, larger sample sizes leads to less variability of the slope coefficient."),
  answer("Smaller sample size, smaller variability around the line, increased range of explanatory variable.", message="No, less variability around the line and increased variability in the explanatory variable both lead to less variability of the slope coefficient."),
  answer("Smaller sample size, larger variability around the line, decreased range of explanatory variable.", correct=TRUE, message="Right!  Recall the more variability in the response and less variability in the explanatory variable *both* increase the variability of the slope coefficient."),
  answer("Smaller sample size, smaller variability around the line, decreased range of explanatory variable.", message="No, less variability around the line leads to less variability of the slope coefficient."),
  allow_retry = TRUE
)
```
