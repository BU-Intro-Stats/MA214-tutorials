...
Now let's compare the two permuted datasets:
...
Note that they are not identical.  Indeed, by permuting the data, we get information about how the line would *vary* if there was not a relationship between IQs in the biological and foster kids.  That is, how does the line vary if the null hypothesis is true and the slope is actually zero?
...
obs_slope <- ___(___, ___) |>
  # Tidy the result
  tidy() |>   
  # Filter for rows where term is Biological
  ___(___) |>
...
In your linear regression, be sure to set `formula` to `Foster ~ Biological` and `data` to `twins`.
</details>

<details>
<summary>**Hint 2**</summary>
In the last line, call `pull()` and pass `estimate`.
</details>
...
In the last line, be sure to set `stat` to `"slope"` when you call `calculate()`.
</details>
...
Repeat the above, but this time simulate 500 slopes.
...
<details>
<summary>**Hint**</summary>
In the first blank line, call `ungroup()` without arguments.
</details>
...
<details>
<summary>**Hint**</summary>
The last line, with `abs()`, needs no arguments.
</details>
...
# Create a confidence interval of the boot_slope stat using
...
<details>
<summary>**Hint**</summary>
Your lower and upper percentile cutoffs should be in terms of alpha, and they should add up to 1.
</details>

<details>
<summary>**Hint**</summary>
To use `quantile()`, pass `stat` and th percentile cutoffs you calculated.
</details>
