...
Lots of things can go wrong if the distribution doesn't match the assumptions.
</details>
...
<details>
<summary>**Hint**</summary>
Recall that the degrees of freedom for the t distribution should be $n-2$. How do you get $n$?
</details>
...
Inside `stat_function()`, set `fun` to `dt`, `args` to a list with `df` set to `degrees_of_freedom`, and `color` set to `"red"`.
</details>
...
- Get the Biological coefficient from the model: first, tidy the model and then filter for the `term` `"Biological"`.
- Add a column, `one_sided_p_value`, for the one-sided p-value.
...
<details>
<summary>**Hint 1**</summary>
Recall that the degrees of freedom is the number of rows of data minus two. How do you get the number of rows?
</details>

<details>
<summary>**Hint 2**</summary>
Q: What is the test statistic here?
</details>

<details>
<summary>**Hint 3**</summary>
To calculate the one-sided p-value, call `pt()`, passing `test_statistic` and setting `df` to `degrees_of_freedom`. The two-sided p-value should be twice the one-sided p-value.
</details>
...
Great, now you have the two-sided p-value for the alternative hypothesis that the true slope is different from 1. Is your result significant?
...
<details>
<summary>**Hint**</summary>
Recall that you can calculate a proportion for `x` values "at least as extreme as `y`" using `mean(x >= y)`.
</details>
...
In the example above, you can see that the bootstrap confidence interval ranges from 11.2 to 14.5 (very close to the mathematical approximation which went from 11.1 to 14.4).
...
<details>
<summary>**Hint 1**</summary>
The upper percentile cutoff should be `1` minus (`alpha` divided by `2`). Why?
</details>

<details>
<summary>**Hint 2**</summary>
Use `qt()` with `p_upper` and `degrees_of_freedom` for computing the `critical_value`.
</details>
...
<details>
<summary>**Hint 2**</summary>
Inside `mutate()`, set `lower` to `estimate` minus `critical_value` times `std.error`. How would you adjust this for computing `upper`?
</details>
...
<details>
<summary>**Hint 2**</summary>
Inside `summarize()`, calculate `lower` by calling `quantile()` with `stat` and `p_lower`. How would you adjust this to calculate `upper`?
</details>
...
Additionally, you can see the intervals which capture the average calorie content with 95% confidence for foods that are 20g of fat (`388.5 and 417.8`) and 30g of fat (`504.9 to 556.6`).
...
<details>
<summary>**Hint**</summary>
Inside `mutate()`, calculate `lower_mean_prediction` as `.fitted` minus `critical_value` times `.se.fit`. How would you adjust this to calculate `upper_mean_prediction`?
</details>
...
You will now manually create what `geom_smooth()` does, using `predictions`. Provide the aesthetics and data to each geom.
...
<details>
<summary>**Hint 1**</summary>
For the aesthetics of `geom_point()`, be sure to map `x` to `Biological` and `y` to `Foster`. Set `data` to `twins`.
</details>

<details>
<summary>**Hint 2**</summary>
For the aesthetics of `geom_line()`, you'll need to instead map `y` to `.fitted` from the `predictions` data. What do you need to set `data` to?
</details>

<details>
<summary>**Hint 3**</summary>
Finally, for the aesthetics of `geom_ribbon()`, you'll need to map `ymin` to `lower_mean_prediction` and `ymax` to `upper_mean_prediction`. What do you need to set `data` to? Don't forget to set `alpha` to 0.2.
</details>
...
<details>
<summary>**Hint**</summary>
Inside `mutate()`, `std_err_of_predictions` should be the `sqrt()` of (`twins_sigma` to the power `2`) plus (`.se.fit` to the power `2`). 
</details>
...
<details>
<summary>**Hint**</summary>
Calculate `lower_response_prediction` as `.fitted` minus `critical_value` times `std_err_of_predictions`. How would you adjust this to calculate `upper_response_prediction`?
</details>
