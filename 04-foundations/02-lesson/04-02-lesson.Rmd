---
title: "Foundations of inference: 2 - Randomization Test"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
Categorical variables are often summarized using proportions, and it is always important to understand the denominator of the proportion. Recall that we want to determine whether the gender of an employee affects their promotion rate, and we have observed a lower promotion rate for women than for men with the same resume. The summary table for our data is included again below:
...
```{r reading-table-mc}
question(
  "Using the table above, what is the proportion we want to model?",
  correct = "Correct! To get the correct proportion in your R code, you'll need to group by `gender` **before** you calculate the proportions!", allow_retry = TRUE,
  answer("The proportion: female employees promoted / total female employees, or 14/24.",
    correct = TRUE),
  answer("The proportion: female employees promoted / female employees not promoted, or 10/14."),  # , message = ""
  answer("The proportion: female employees promoted / total employees promoted, or 14/35."),  #, message = ""
  answer("The proportion: female employees promoted / male employees promoted, or 14/21."))  #, message = ""
```

<!-- ```{r prop-denom-mc} -->
<!-- question("Which value do we want in the denominator of the proportion?", allow_retry = TRUE, -->
<!--     answer("The proportion of women who were promoted.", message = "xxx"), -->
<!--     answer("The proportion of promoted individuals who were women.", correct = TRUE), -->
<!--     answer("The proportion of men who were not promoted.", message = "xxx"), -->
<!-- ) -->
<!-- ``` -->
...
- Then calculate the proportion promoted for each gender and call the summary variable `prop_promoted`.
- Finally, calculate the difference in proportions of promoted between genders by summarizing again, setting `stat` to the `diff()` of `prop_promoted`.
...
Draw a histogram of the permuted differences:

- Using the permutation dataset `gender_discrimination_perm`, plot the distribution of test statistics (`stat`).
...
* Difference in proportions: ${\widehat p} - {p}$

* Ratio $\frac{\widehat p}{p}$
...
<!-- Note: quantiles listed below look right to me -->
...
The `quantile()` function often comes in handy! It is a function from "base R" that calculates the quantile of a numerical variable, where the quantile is specified by the `probs` argument. By specifying  `probs = 0.05`, you are telling R that you are interested in the value of `stat` where 5% of the data points are below it.. Similarly for `probs = 0.95`, where 95% of the values of `stat` are below it. 
...
We've created two new datasets for you with essentially the same difference in proportions as the original discrimination data. However, one of the datasets (`gender_discrimination_small`) is one third the size of the original dataset and the other (`gender_discrimination_big`) is 10 times larger than the original dataset.
...
Create a contingency table of the gender and promotion variables for both the small dataset `gender_discrimination_small` and `gender_discrimination_big`. 

<!-- Tabulate the small dataset, `gender_discrimination_small`. That is, call `count()`, passing the `gender` and `decision` columns, to get a contingency table. Then do the same with the big dataset, `gender_discrimination_big`. -->

```{r sample_size, exercise=TRUE}
# Tabulate the small dataset
gender_discrimination_small |> 
  # Count gender and promote
  ___(___, ___)
...
Recall that we use `count()` to create contingency tables. What should the inputs be here?
</details>
...
- Add a vertical line using `geom_vline()`, with x intercept `diff_orig_small`.
...
Draw the same plot again, this time using the big dataset `gender_discrimination_big_perm` and an x intercept of `diff_orig_big`.
...
ggplot(___, aes(x = stat)) + 
  ...
  geom_vline(aes(___ = ___), color = "red")
```
...
```{r diff-samples-mc}
question("What can you conclude from the sample size exercise?", allow_retry = TRUE,
    answer("The observed difference is consistent with differences you would see by chance for both small and large sample sizes.", message = "Unfortunately no - try re-reading the graphs you created above."),
    answer("The observed difference is consistent with differences you would see by chance if the sample size was small, but it would virtually never be observed by chance if the sample size was big.", correct = TRUE)
)
```  
...
Now using the randomization distributions with the small and big datasets, you will calculate different cutoffs for significance. Remember, you are most interested in a large positive difference in promotion rates, so you are calculating the upper quantiles of 0.90, 0.95, and 0.99.

A function for calculating these quantiles, `calc_upper_quantiles()` is shown in the script. We don't expect for you to learn how to write a function in R! Instead, the function is intended to demonstrate how we can automate the process of finding the 90%, 95%, and 99% quantiles of a test statistic.  
...
Recall that the 0.1 quantile is 0.125, the 0.05 quantile is 0.208, and the 0.01 quantile is 0.292. So, depending on the level of significance, we may or may not reject the null hypothesis. To get around the "sometimes reject" problem, we quantify the degree to which the data disagree with the null distribution using something called a p-value.
...
In this example, the p-value is the _probability of observing a difference of 0.2917 or greater assuming that promotion rates do not vary across gender_.
...
We can also use the `visualize()` and `shade_p_value()` functions to visualize where the observed statistic falls in the distribution of permuted statistics, and shade the direction that the p-value was calculated from. The `shade_p_value()` function has many inputs (find out more by typing `?shade_p_value` in your console), but the most important ones are the __same__ as the `get_p_value()` function!  
...
You can test out the different methods for calculating the p-value by trying out: `direction = "greater"`, `direction = "two_sided"`, and `direction = "less"`. Arguments of both `shade_p_value()` and `get_p_value()` functions should be `obs_stat = diff_orig, direction = "greater"`, but remember to use the correct dataset!
</details>

<details>
<summary>**Hint 2**</summary>
For this exercise, arguments of both `shade_p_value()` and `get_p_value()` functions should be `obs_stat = diff_orig, direction = "greater"`, but remember to use the correct dataset!
</details>
...
Well done! You may have noticed that the observed statistic is borderline "significant" (p-value = 0.025) for the original data. However, the same statistic would have been very consistent with the small dataset (p-value = 0.281) and extremely unusual for the large dataset (p-value $\approx$ 0).
...
The `gender_discrimination_new_perm` and `diff_orig_new` datasets are also available in your workspace. At the end, you'll be comparing the p-values and distributions for the different datasets.


**Exercise**

Get the counts of `gender` and `decision` for both the original dataset, `gender_discrimination`, and the new dataset, `gender_discrimination_new`.
...
<!-- ```{r pvalue_2-hint} -->
<!--  Change the data frame to `gender_discrimination_new`. -->
<!-- ``` -->
...
Create a histogram for both the original permutated dataset, `gender_discrimination_perm`, and the new permuted dataset, `gender_discrimination_new_perm`. On the second plot, use `geom_vline()` to specify a red x-intercept of `diff_orig_new`.

```{r pvalue_3, exercise=TRUE}
# Recall the distribution of the original permuted differences
___

# Plot the distribution of the new permuted differences with a red x-intercept
___
```
...
First, re-compute the p-value of the origianl permutation dataset, `gender_discrimination_perm`. Then compute the p-value of the new dataset `gender_discrimination_new_perm` using the new observed difference, `diff_orig_new`.
...
```{r diff-histograms-mc}
question("Why do the histograms above for the permutation differences look the same, even when we have different data?", correct = "Correct!", allow_retry = TRUE,
  answer("The null hypothesis is the same when we're testing on either dataset.",
    message = "What happens if the null hypothesis is the same, but the dataset size is different?"),
  answer("The null hypothesis is the same when we're testing on either dataset, and the dataset sizes are the same.",
    message = "What happens if both of these are true, but the number of employees promoted is different?"),
  answer("The null hypothesis is the same when we're testing on either dataset, and the number of employees promoted is the same.",
    message = "What happens if both of these are true, but the dataset sizes are different?"),
    answer("The null hypothesis is the same when we're testing on either dataset, the dataset sizes are the same, and the number of employees promoted is the same.",
    correct = TRUE))
```
...
<!-- Does the null hypothesis depend on the dataset, or is it the same for both?  -->
