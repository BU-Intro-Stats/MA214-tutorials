...
$$\widehat{p}_W - \widehat{p}_E = \frac{2}{3} - \frac{2}{3} = 0$$

![](images/lesson1_img5.png){ width=80% }
$$\widehat{p}_W - \widehat{p}_E = \frac{1}{3} - \frac{2}{3} = -\frac{1}{3}$$

As a way of summarizing each of the null samples, we calculate one statistic from each sample. Here, the statistic is the difference in the proportion of West Coast people who prefer cola as compared with the proportion of East Coast people who prefer cola, where each of the sample proportions is denoted “$\widehat{p}$”. The difference in $\widehat{p}$s changes with each sample. First it is 0, then it is negative one third, and it will keep changing.
...
The original sample proportions are $\widehat{p}_{East}$ of 0.82 and $\widehat{p}_{West}$ of 0.73. A difference of -0.09. 
...
Note that the code `drink == "Cola"` produces a vector of `TRUE`s and `FALSE`s, which R then coerces to ones (TRUE) and zeros (FALSE) when the `mean()` function is applied. Since a TRUE (one) represents an individual who prefers cola, the average of these ones and zeros represents the proportion of individuals who prefer cola.
...
The `rep_sample_n()` function performs repeated sampling of a dataset, where the `size` of the sample is specified in the first argument and the number of repetitions is specified in the `reps` argument. We notice that the sample `size` is the same size as the soda dataset, since it equals the number of rows in the original dataset. We are also specifying that the sampling of the dataset should be done without replacement (`replace = FALSE`), since we want each row to only be selected once. You can think of this as creating five copies of the original `soda` dataset. 
...
Great work!  Notice that the grey portions of the bars indicate a large number of missing observations in the dataset.  If this were your dataset, it would be important to stop here and consider the cause of the missingness.  For the tutorial, we will now proceed without worrying about the missing observations.
...
From `NHANES`, select the `Gender` and `HomeOwn`ership columns, then filter the rows to keep individuals whose home ownership status is `"Own"` or `"Rent"`.  Save the result to the dataset named `homes`. Then, find the number of rows and columns in `homes` with `dim()`.
...
# Find the number of rows and columns of homes
___(___)
```

<details>
<summary>**Hint**</summary>
Hint: `dim(x)` gives the full dimensions of object `x`. If `x` has n rows and m columns, the output will be `[1] n m`.
</details>
...
<!-- ```{r statistic_of_interest-hint-4} -->
...
2. Summarize the proportion of individuals who own a home. *Note that this will calculate proportions for both genders since you've grouped by the `Gender` variable in the line before it.*
3. Finally, summarize the observed difference in proportions using `diff()` on the calculate proportion.
...
In steps 2 and 3, you should be using `summarize()` to summarize the data. 
</details>

<details>
<summary>**Hint 2**</summary>
In step 2, try taking the `mean()` of observations with `"Own"` to calculate the proportion. 
</details>
...
<!-- - `hypothesize()` that the null is true where `null = "independence"` (gender and homeownership are not related); and finally -->
...
<details>
<summary>**Hint**</summary>
Refer to the example above, replacing the variable names with the variables in your homes dataset. The syntax will be the same.
</details>

<!-- ```{r null_model-hint} -->
<!--  `specify()` takes a formula, `HomeOwn ~ Gender`, and `success` is `"Own"`. -->
<!-- ``` -->
...
<!-- `specify()` has been done for you. Using the infer framework,  -->
...
Using the `homeown_perm` permutations you generated above, `calculate()` the statistic `"diff in props"` with the order of `c("male", "female")`. Setting the order in this way tells `R` that the female proportion should be subtracted from the male proportion.
...
<details>
<summary>**Hint**</summary>
The function `calculate()` takes input like: `calculate(stat="[statistic of interest]")`. What's your statistic of interest here?
</details>

<!-- ```{r randomized_statistics-hint} -->
...
Generate 1000 differences in proportions by shuffling the `HomeOwn` variable using the infer syntax. Then, run the density plot code to create a smoothed visual representation of the distribution of differences. **What shape does the curve have?**
...
<details>
<summary>**Hint**</summary>
Since you observed only 212 of 1000 null statistics are more extreme than the observed difference statistic, you can conclude that the observed difference **is** consistent with the permuted distribution. You've failed to reject the null hypothesis!
</details>
