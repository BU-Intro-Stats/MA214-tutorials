...
```{r hypotheses-mc}
question(
  "Which of the following null and alternative hypotheses are appropriate for the opportunity cost study?", correct = "Correct!", allow_retry = TRUE,
  answer("H0: Being reminded that they could save money instead of buying a DVD did not influence the students' decision. HA: The students in the control (no reminder) group were more likely to save their money.",
    message = "Not quite, the researchers are expecting to see more decisions to save in a particular direction (the alternative hypothesis)."),
  answer("H0: The students in the treatment (reminder) group were more likely to save their money. HA: Being reminded that they could save money instead of buying a DVD did not influence the students' decision.",
    message = "Remember, the null hypothesis always contains the claim that there is no effect of the variables."),
  answer("H0: Being reminded that they could save money instead of buying a DVD did not influence the students' decision. HA: The students in the treatment (reminder) group were more likely to save their money.", correct = TRUE),
  answer("H0: The students in the control (no reminder) group were more likely to save their money. HA: Being reminded that they could save money instead of buying a DVD did not influence the students' decision.",
    message = "Remember, the null hypothesis always contains the claim that there is no effect of the variables."))
```
...
We're interested in whether the treatment and control groups were equally likely to buy a DVD after reading the experimental statements. In this exercise, you'll use the data from the study to find the sample statistics (here: proportions) that are needed for the analysis.
...
As in lesson 2, you will permute the data to generate a distribution of differences as if the null hypothesis were true. As a reminder, here's what the data looked like:
...
# keep in workspace so they can complete part 3
opp_perm <- opportunity |>
  specify(decision ~ group, success = "buyDVD") |>
  hypothesize(null = "independence") |>
  generate(reps = 1000, type = "permute") |>
...
# Using the permutation data, plot stat
...
- First `visualize()` the sampling distribution of the permuted statistics indicating the place where `obs_stat = diff_obs`, and coloring in values below with the command `direction = "less"` using `shade_p_value()`. [this is already done for you below]
...
```{r visualize_opp_perm, echo = TRUE, show = TRUE}
...
<!-- *TODO: error `EXPR must be a length 1 vector`* -->
...
Great work! The small p-value indicates that the observed data are inconsistent with the null hypothesis.  We should reject the null claim and tentatively conclude that financial advice does affect the likelihood of purchase.
...
This should be double what you calculated as a one-tailed p-value before.
</details>
...
```{r reflect-mc}
question("Based on the plot you created in the last exercise (displayed again here), which of the following seems like a reasonable conclusion about opportunity cost?",
  correct = "Good job! There are very few permuted differences which are as extreme as the observed difference, and we can make a causative conclusion because the study was experimental, with randomized groups.", allow_retry = TRUE,
  answer("In the population there is no evidence that reminding people of their savings will influence their buying decisions.", message = "No, try again. Look at the curves and the p-values to see if the observed data is consistent with the null hypothesis."),
  answer("In the population there is evidence that reminding people of their savings does influence their buying decisions, and there is also evidence that it is the reminder itself that causes the difference in buying decisions.", correct = TRUE),
  answer("In the population there is evidence that reminding people of their savings does influence their buying decisions, but we cannot tell whether the difference is due to the reminder itself or something else.", message = "Incorrect. This would be true if the study had been observational, but in this case we know the data came from a controlled experiment and used randomized groups to control for other factors."))
```
