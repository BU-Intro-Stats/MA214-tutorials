...
Recall from the last round of tutorials that we often use hypothesis tests to answer research questions. Until now, you have used p-values to determine whether there is sufficient evidence to reject a null hypothesis - but there are also other tools you can use, some of which we will learn today. 
...
For each of the estimation problems, we need to understand what a parameter is. A parameter is a _numerical value from the population_. 
...
```{r quiz_1}
quiz(question("Before investigating the sampling variability, what is the population parameter of interest?", correct = "Right!", allow_retry = TRUE,
    ...
    answer("The proportion of all voters in your town who will vote for Candidate X on election day.", correct = TRUE)), caption = "")
```
...
A university is trying to determine whether parking is a problem on its campus, but would only implement a change if it's clear that a majority of students think it's a problem. The student newspaper contacts a random sample of 200 students and asks whether or not they are frustrated with the parking situation.

```{r hypotheses-mc}
question("Which of the following null and alternative hypotheses are appropriate for the campus parking survey? Let p be the proportion of students who are frustrated with parking on campus.", correct = "Correct!", allow_retry = TRUE,
  answer("H0: p = 0.5. HA: p > 0.5.", correct = TRUE),
  answer("H0: p = 100. HA: p > 100", message="Not quite. Is the parameter of interest a count or a proportion?"))
```

Say that the survey reveals that between 50% and 70% of students are frustrated with the parking situation on campus. Do you think this is enough for the university to make a change? 

What if the interval was between 40% and 60%? How sure would you need to be about the range, if you wanted to convince the university? 

We'll answer these questions and more throughout the lesson.
...
Note that the *statistic* at hand is $\widehat{p}$, which is the proportion of successes in the _sample_. The *parameter*, on the other hand, is the proportion of successes in the _population_.
...
We expect the sample statistic to vary around the parameter, but how far is the statistic from the parameter? Bootstrapping is a method that allows us to estimate this distance. Let's see how it works.

![](images/lesson4_img2.png){ width=80% }

Bootstrapping is a method of taking samples with replacement from the observed data. It turns out that the process of resampling from the original sample is an excellent approximation for sampling from a population! 

We call the bootstrapped statistic $\widehat{p}^*$, which is the proportion of successes in the resample.
...
By repeating the resampling process many times, the resampled proportions give a measure of how $\widehat{p}$ varies. 


### Standard error


The **standard error**, which describes how variable the statistic is around the parameter, is key to building a confidence interval. To evaluate how well the bootstrap works, we set up a hypothetical situation where we actually know the number we need. 

<!-- The following R code calculates the standard error of the sample proportions in two ways. First, we use the population information, then the bootstrap, and compare the two. -->
...
<!-- ### Variability of $\widehat{p}$ from the sample (bootstrapping) -->

<!-- To start, consider a totally unrealistic situation on voter preference, where we actually know the true population parameter is that 60% of people prefer candidate X. Then we take bootstrap samples from the observed data. For each of the samples, $\widehat{p}$ is calculated.  -->

<!-- The standard error is exactly the number we need to know and is calculated to be 0.087. But it is unrealistic to think we could take many samples from the population. Typically, the researcher has exactly one sample from the population. -->

<!-- It turns out that the variability of the statistic $\widehat{p}$ can be measured without taking repeated samples from the population. Instead, we take repeated samples from the original observed data. -->
...
<!-- Notice that the resampled $\widehat{p}^*$ values have a standard deviation of 0.084, which is very close to the number 0.087 from the previous, unrealistic slide. -->
...
First, select one poll from which to resample, as `one_poll`.

- Filter out data for every poll except the first poll. That is, only keep `poll` when `poll` equals `1`.
- Select the `vote` column.

Then, compute $\widehat{p}^*$ for each resampled poll, as `ex2_props`, using tolls from the infer package.
...
For selecting one poll, try using `filter()` with the appropriate input. What do you want the value of `poll` to be?
</details>

<details>
<summary>**Hint**</summary>
For computing p-hat* for each resampled poll, recall the `infer` framework: first use `specify()`, then `generate()`, and then `calculate()`. 
</details>
...
- Using `ex1_props`, calculate the variability of $\widehat{p}$. In the call to `summarize()`, set `variability` to the standard deviation, `sd()`, of `stat`.
- Do the same with `ex2_props` to calculate the variability of $\widehat{p}^*$.
...
### Visualizing the variability of $\widehat{p}$

In order to compare the variability of the sampled $\widehat{p}$ and $\widehat{p}^*$ values in the previous exercises, it is valuable to visualize their distributions. To recall, the exercises walked through two different experiments for investigating the variability of $\widehat{p}$ and $\widehat{p}^*$:
...
- Combine data from both experiments using the function `bind_rows()`, with inputs `ex1_props` and `ex2_props`. Call this new dataset `both_ex_props`. The code specifies that an ID column named `experiment` should be created.
...
```{r variability_phat-setup, echo=FALSE}
# Select one poll from which to resample
one_poll <- all_polls |>
  filter(poll == 1) |>
  select(vote)
...
<details>
<summary>**Hint**</summary>
Use the function `bind_rows(<object1>, <object2>)` to combine data from both experiments.
</details>
...
Note that the curves are quite similar in shape. The sampled $\widehat{p}$ values are centered around the true (typically **unknown** parameter) parameter (green); the resampled $\widehat{p}^*$ values are centered around the estimate from the very first poll (red). Great work!

### Always resample the original number of observations

In the bootstrap examples, exactly 30 observations have been repeatedly resampled from the original sample.  The choice of 30 was given because the original sample had 30 observations.  If we had resampled 3 observations instead, the resampled $\widehat{p}^*$ value could have ranged from 0 to 1 (producing a much larger $SE(\widehat{p}^*)$ than desired).  If we had resampled 300 observations instead, the resampled $\widehat{p}^*$ value would have been close to the same number each time (producing a much smaller $SE(\widehat{p}^*)$ than desired).
...
question("Generally, if $n$ represents the size of the original sample, how many observations should we resample with replacement when bootstrapping?",
  ...
  answer("Resample exactly $n$ observations because then the variability of $\\widehat{p}^*$ will be most similar / representative of the original sampling process.", correct = TRUE),
...
## Variability in $\widehat{p}$

Remember the goal of creating a confidence interval is to find a range of plausible values for the true population parameter. The observed statistic is a good estimate, but it is a random variable and varies from sample to sample. As such, we can't know how far away the statistic $\widehat{p}$ is from the true parameter $p$ just by looking at our observed data.  We can, however, get a sense for the distance between the statistic and the parameter by looking at how much the re-sampled proportions ($\widehat{p}^*$) vary.

<!-- Remember the goal of creating a confidence interval is to find a range of plausible values for the true population parameter. The observed statistic is a good estimate, but it is impossible to know whether it is one of the $\widehat{p}$ values, which is very close to the true parameter or whether it is one of the $\widehat{p}$ values, which is in the tail of the distribution of observed statistics. We won't know how far away the statistic is from the parameter just by looking at the sample of data.  We can however, get a sense for the distance between the statistic ($\widehat{p}$) and the parameter (p) by looking at how much the re-sampled proportions ($\widehat{p}^*$) vary. -->

<!-- The image below shows that the re-sampled proportions center around $\widehat{p}$, but the true proportion may be one of the values near $\widehat{p}$ (red) or one of the values in the tail (blue). -->
...
The same variability can be measured through a different mechanism.  As before, if $\widehat{p}$ is sufficiently close to the true parameter, then the resampled (or bootstrapped) $\widehat{p}^*$ values will vary in such a way that they overlap with the true parameter.  

Instead of using $\pm 2 SE$ as a way to measure the middle 95% of the sampled $\widehat{p}$ values, you can find the middle of the resampled $\widehat{p}^*$ values by removing the upper and lower 2.5%.  Note that this second method of constructing bootstrap intervals also gives an intuitive way for making 90% or 99% confidence intervals as well as 95% intervals.
...
Calculate the `95` percent interval of the bootstrapped $\widehat{p}^*$ values contained in `one_poll_boot`:
...
<details>
<summary>**Hint**</summary>
Inside summarize, use `quantile()` with the correct quantile level to get the upper and lower values.
</details>
...
The previous bootstrapped $\widehat{p}^*$ values have been loaded for you and are available in `one_poll_boot`.
...
The results you just got have been stored in a dataframe called `conf_int_data`. 
- With this dataset, plot `ci_endpoints` (y-axis) vs. `ci_percent` (x-axis). 
...
Recall that our goal with bootstrapping is to estimate the *sampling distribution* of the random variable $\widehat{p}^*$ using our observed polling data. When we resample the `poll`s with replacement $B$ times, we create a *bootstrap distribution* that, for a large number of bootstrap resamples, will satisfy the Central Limit Theorem. 

Formally, say that our `poll` data consist of $n$ Bernoulli samples: $X_1,...,X_n \sim Bernoulli(p)$ where $p$ is the population parameter we wish to estimate. Here, a "success" is a given person's stated support for candidate X in the poll. Then our point estimate is given by

$$\widehat{p} = \frac{1}{n}\sum_{i=1}^n X_i$$

And the bootstrapping procedure creates $B$ independent resamples of $X_1,...,X_n$:

$$\mathbf{X}_1^*=(X_{1,1}^*,...,X_{1,n}^*),...,\mathbf{X}_B^*=(X_{B,1}^*,...,X_{B,n}^*)$$
so for each of the $B$ bootstrap samples we compute a new sample proportion:

$$\widehat{p}_1^* = \frac{1}{n}\sum_{i=1}^n X^*_{1,i} \ ,..., \widehat{p}_B^*= \frac{1}{n}\sum_{i=1}^n X^*_{B,i}$$

We say these bootstrapped sample proportions $\widehat{p}^*$ follow a bootstrap distribution, which approximates the sampling distribution of $p$. Then for large $B$, the Central Limit Theorem says that:

$$\widehat{p}^* \sim \mathcal{N}\biggl(p, \frac{p(1-p)}{n} \biggr)$$ 

or in words, is approximately normally distributed with mean $p$ and variance $p(1-p)$. 

###

To bring us back to our problem, the following plot shows $B=200$ bootstrap resamples of the `poll` data. The red and blue dots each represent two different point estimates for $p$. 
...
all_polls |>
  ...
  mutate(pointer=c(rep("col1", 90), "col2", rep("col1", 108), "col3")) |>
ggplot() +
...
```{r clt-mc}
question("Based on the plot of the bootstrap distribution above and what you know from the CLT, where does the population proportion p lie?",
  correct = "Good job! The CLT says that for large resamples B, the bootstrap distribution is roughly normal and is centered at the population proportion.", allow_retry = TRUE,
  answer("The blue dot, around p=0.82", message = "No, try again. The blue dot is in the tail of the bootstrap distribution."),
  answer("The red dot, around p=0.56.", message = "Incorrect. The red dot is towards the center of the bootstrap distribution but not quite there. Where should the mean be according to the CLT?"),
  answer("Neither the blue nor red dot, but instead around p=0.6.", correct = TRUE))
```

<!-- The variability of the $\widehat{p}$ statistics gives a measure for how far apart any given observed $\widehat{p}$ and the parameter are expected to be.  -->


<!-- ### Standard error of $\widehat{p}$ -->


<!-- Note that bootstrapping, or resampling with replacement the same number of observations as were in the original sample, provided approximately the same standard error as sampling many $\widehat{p}$ values from the population. The standard error is measured by the width of the distribution and here we can see that the green bootstrap distribution is approximately the same width as the black distribution, which measured the true variability of $\widehat{p}$ values from the population. -->
...
<!-- We can use the value of the standard error to count the number of sample $\widehat{p}$s, which are close to the parameter. -->

<!-- It turns out that because the distribution of $\widehat{p}$ values is symmetric and bell shaped, roughly 95% of samples will produce $\widehat{p}$s that are within two standard errors of the center. This idea is called the **empirical rule** and comes from theory which describes bell-shaped distributions. -->
...
One advantage of taking many bootstrap samples is that, if the bootstrapping distribution is approximately normal via the CLT, then we can use the Empirical Rule to describe the variability of the realizations. 
...
<!-- One such property is that if the variability of the sample proportion (called the *standard error*, or $SE$) is known, then approximately 95% of $\widehat{p}$ values (from different samples) will be within $2SE$ of the true population proportion. -->

The Empirical Rule, also called the 68-95-99.7 Rule, says that: 

- 68% of the data fall within 1 *standard error* or $SE$ of the mean;
- 95% of the data fall within 2 $SE$ of the mean; and
- 99.7% of the data fall within 3 $SE$ of the mean,

where the mean is the value of the true parameter. Recall that in the plot of the bootstrap distribution in the previous section, the blue dot in the tail of the distribution was far from the mean while the red dot was much closer.

###

Here, in red, we see all the sample $\widehat{p}$ values that are close to the true parameter. If our data is one of the red dots, our confidence interval will correctly capture the true parameter.
...
The `all_polls` dataset contains $B=1000$ samples of size 30 from a population with a probability of voting for Candidate X equal to 0.6.
...
- Generate `props`, the proportion of individuals who are planning to vote yes in each poll.
...
1. You can measure the variability associated with $\widehat{p}$ by resampling from the original sample.  
2. Once you know the variability of $\widehat{p}$, you can use it as a way to measure how far away the true proportion is.  
...
- Calculate $\widehat{p}$ and assign the result to `p_hat`. In the call to `summarize()`, calculate `stat` as the `mean()` of `vote`s that equal `"yes"`.
- Find an interval of values that are plausible for the true parameter by calculating $\widehat{p} \pm 2SE$.
...
Remember that a confidence level describes how likely you are to have gotten a sample that was close enough to the true parameter. Indeed, with the sample at hand, the confidence interval of plausible values *does* contain the true population parameter of 0.6. Good work.
...
<!-- By using the bootstrapped $\widehat{p}$ values in the previous exercises, you were able to find two different intervals for the true parameter of interest, the proportion of the population who will vote for candidate X. -->
...
<!-- The second interval used the natural variability of the resampled $\widehat{p}$ values to characterize the distance from the true parameter. Remember, as in previous exercises, the quantile function finds the value which gives 2.5% of the bootstrapped proportions below and the value which gives 97.5% of the bootstrapped proportions below. -->
...
<!-- Ideas about the variability of $\widehat{p}^*$ are used to estimate how far the observed $\widehat{p}$ was from the population proportion. -->
...
<!-- A plot of the bootstrap $\widehat{p}^*$ values will give a good indication for whether the technical conditions are valid. -->
...
<!-- A function for calculating the bootstrapped t-confidence interval, `calc_t_conf_int()`, is shown is the script. Read the code and try to understand what it is doing. You can use the function by typing `calc_t_conf_int()` in the code. Then: -->
...
<!-- One additional element that changes the width of the confidence interval is the sample parameter value, $\widehat{p}$.   -->
<!-- Generally, when the true parameter is close to 0.5, the standard error of $\widehat{p}$ is larger than when the true parameter is closer to 0 or 1.  When calculating a bootstrap t-confidence interval, the standard error controls the width of the CI, and here (given a true parameter of 0.8) the sample proportion is higher than in previous exercises, so the width of the confidence interval will be narrower. -->


<!-- **Exercise:** -->

<!-- The function `calc_p_hat()` is shown in the script, and can be used to calculate the sample proportions. The `calc_t_conf_int()` function from the previous exercise has been updated to now use any value of `p_hat` as an argument! Read the function and try to understand what it's doing. Then: -->
...
<!-- - Calculate $\widehat{p}$ of this new sample, using the same technique as with the original dataset. Assign it to the variable `p_hat_0.8`. -->
<!-- - Find the bootstrap t-confidence interval using the new bootstrapped data, `one_poll_boot_0.8`, and the new $\widehat{p}$. *Notice that it is narrower than previously calculated.* -->
