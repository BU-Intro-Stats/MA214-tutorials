...
### Spam and exclamation marks!

Let's look at a more obvious indicator of spam: exclamation marks. The `exclaim_mess` variable contains the number of exclamation marks in each message. Using summary statistics and visualization, see if there is a relationship between this variable and whether or not a message is spam.
...
- If you decide to use a log transformation, remember that `log(0)` is `-Inf` in R, which isn't a very useful value! You can get around this by adding `1` to the quantity inside the `log()` function. This way, values of 
zero will be mapped to zero since `log(1 + 0) = log(1) = 0`, which helps with interpretation. 
This is a commonly used trick. 
Sometimes a number other than `1` is added, but this can be more difficult to interpret. 
- Another recommendation is to use `log10` (log base 10) rather than `log` (the natural log) because
base 10 is easier to interpret. 
...
# Create plot for spam and exclaim_mess
email |>
  mutate(log_exclaim_mess = log10(1 + exclaim_mess)) |>
...
",
  answer("The most common value of `exclaim_mess` in both classes of email is zero."),
...
When the number of observations in the histograms is very different, which is somewhat the 
case when comparing non-spam to spam, it can become more difficult to interpret. 
For example, we cannot easily read off the relative probabilities of the histogram bins. 
So accomplish that, we need to normalize the bin heights.
This can be accomplished by setting the `y` aesthetic to `after_stat(density)`. 

```{r ex2-density, exercise = TRUE}
# Create plot for spam and exclaim_mess with normalized density histograms 
...
```{r mc2-density}
question(
  "Which interpretation of these normalized faceted histograms is NOT correct?
",
  answer("The most common value of `exclaim_mess` in both classes of email is zero."),
  answer("There are more cases of spam in this dataset than not spam.", correct = TRUE, message = "Well done!"),
  answer("Even after a transformation, the distribution of `exclaim_mess` in both classes of email is right-skewed."),
...
The dominant feature of the exclaim mess variable, though, is the large proportion of cases that report zero  exclamation marks. This is a common occurrence in data analysis that is often termed "zero-inflation", and there are several common ways to think about those zeros.

```{r}
p1 <- email |> 
  mutate(log_num_char = log10(num_char)) |> 
...
email2 <- email |>
  mutate(zero = exclaim_mess == 1, log_exclaim_mess = log10(exclaim_mess))
