...
```{r quiz-sampling-experimental-attributes}
quiz(
...
Using the `names()` function we can obtain a list of the variables. 

```{r names-evals, echo=TRUE}
# List of the variables in the data
names(evals)
```

The third variable is the average evaluation score of the professor for a given course. Scores range between 1 and 5 with 1 being a poor evaluation and 5 being an excellent evaluation.
...
Use the `glimpse()` function to explore the `evals` data once again with the following goals in mind: (1) Identify each variable as numerical or categorical. (2) If numerical, determine if it is discrete or continuous. (3) If categorical, determine if it is ordinal or not.


```{r inspect-variable-types, exercise=TRUE}
# Inspect variable types using glimpse

```

Using this information, identify the following variable types:

```{r quiz-variable-type-for-score}
quiz(
  question("What is the type of the variable 'score'?", correct = "Good!", allow_retry = TRUE,
    answer("Continous numerical", correct = TRUE),
    answer("Discrete numerical", message = "Incorrect!"),
    answer("Nominal categorical", message = "Try again!"),
    answer("Ordinal categorical", message = "Nope!")
    ), caption = "")
```

```{r quiz-variable-type-for-age}
quiz(
  question("What is the type of the variable 'age'?", correct = "Nice work!", allow_retry = TRUE,
    answer("Continous numerical", message = "Incorrect!"),
    answer("Discrete numerical", correct = TRUE),
    answer("Nominal categorical", message = "Try again!"),
    answer("Ordinal categorical", message = "Nope!")
    ), caption = "")
```

```{r quiz-variable-type-for-cls-level}
quiz(
  question("What is the type of the variable 'cls_level'?", correct = "You got it!", allow_retry = TRUE,
    answer("Continous numerical", message = "Nope!"),
    answer("Discrete numerical", message = "Incorrect!"),
    answer("Nominal categorical", message = "Try again!"),
    answer("Ordinal categorical", correct = TRUE)
    ), caption = "")
```
...
```{r type-of-variable-hint}
# Condition for "midsize" should be:
cls_students >= 19 & cls_students <= 59 
```
...
### Create a scatterplot for large classes only

The previous plot is a little messy. Suppose you want to just plot the results for large classes. 

- Use `filter()` to select only large classes and save the results in a new data frame called `evals_large`

- Use `ggplot()` with `evals_large` to create a scatterplot displaying the relationship between the variables for just the large classes.  

```{r create-scatterplot-large, exercise=TRUE}
# Filter for large classes only
evals_large = evals |> filter(___)

# Scatterplot of score vs. bty_avg using evals_large
___
```

<!-- ```{r create-scatterplot-large-solution} -->
<!-- # Scatterplot of score vs. bty_avg -->
<!-- evals_large = evals |> filter(cls_type == "large") -->

<!-- # Scatterplot of score vs. bty_avg using evals_large -->
<!-- ggplot(evals_large, aes(x = bty_avg, y = score)) + -->
<!--   geom_point() -->
<!-- ``` -->
